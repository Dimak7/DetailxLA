      ...
      {section === "schedule" && (
        <>
          <section className="paper">
            ...
            <div className="staff-calendar" role="grid" aria-label="Weekly staff calendar"><div className="calendar-axis">TIME</div>{rosterDays.map((day) => { const shifts = scheduleShifts.filter((shift) => s(shift.shift_date) === day && s(shift.status) === "scheduled"); const target = n(rows(data.staffing).find((item) => s(item.staffing_date) === day)?.required_staff); return <div className="calendar-head" key={day}><strong>{new Date(day + "T12:00:00Z").toLocaleDateString("en-US", { weekday: "short", day: "numeric", timeZone: "UTC" })}</strong><button onClick={() => staffingEdit(day, target)}>Staff {shifts.length}/{target || "-"}</button></div>})}<div className="calendar-times">{Array.from({ length: 12 }, (_, hour) => <span key={hour}>{timeLabel((hour + 8) * 60)}</span>)}</div>{rosterDays.map((day) => <div className="calendar-day" key={day} onClick={(event) => { if (event.target === event.currentTarget) shiftEdit({ shift_date: day, start_minute: 540, end_minute: 1020 }); }}>{scheduleShifts.filter((shift) => s(shift.shift_date) === day && s(shift.status) === "scheduled").map((shift, index) => <button className="shift-event" key={s(shift.id)} style={{ top: `${Math.max(0, (n(shift.start_minute) - 480) / 2)}px`, height: `${Math.max(38, (n(shift.end_minute) - n(shift.start_minute)) / 2)}px`, left: `${6 + (index % 2) * 48}%` }} onClick={(event) => { event.stopPropagation(); shiftEdit(shift); }}><strong>{s(shift.name)}</strong><small>{s(shift.position)}</small><span>{timeLabel(n(shift.start_minute))} - {timeLabel(n(shift.end_minute))}</span></button>)}</div>)}</div>
          </section>
