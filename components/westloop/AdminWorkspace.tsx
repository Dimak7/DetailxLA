    ...
    <>
      ...
      {section === "payroll" && (
        <section className="paper">
          ...
          <p className="small-note">Internal earnings estimate based on actual clocked hours and completed jobs. It is not a tax withholding or payroll processor.</p>
          {table(["Employee", "Hours", "Hourly", "Commission", "Jobs", "Revenue", "Total earnings"], list.map((r) => [<><strong>{s(r.name)}</strong><small>{s(r.position)} · {title(s(r.compensation_model))}</small></>, (n(r.minutes) / 60).toFixed(2) + "h", money(n(r.hourly_earnings_cents)), money(n(r.commission_cents)), s(r.jobs_completed), money(n(r.attributed_revenue)), money(n(r.total_earnings_cents))]))}
