  ...
  return (
    <div className="admin-shell">
      ...
      <div className="app-body">
        <aside className="admin-sidebar"><nav aria-label="Business workspaces">{workspaces.filter((workspace) => sections.includes(workspace.target)).map((workspace) => <Link key={workspace.target} href={`/admin/${workspace.target}`} className={activeWorkspace?.target === workspace.target ? "active" : ""}>{workspace.label}</Link>)}<Link href="/admin/settings" className={currentSection === "settings" ? "active" : ""}>Settings</Link></nav></aside>
